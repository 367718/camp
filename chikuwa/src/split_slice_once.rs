pub fn split_slice_once<'c>(content: &'c [u8], separator: &[u8]) -> (&'c [u8], &'c [u8]) {
...
mod tests {
    ...
    fn single() {
        ...
        let output = split_slice_once(content, separator);
    ...
    fn emoji() {
        ...
        let output = split_slice_once(content.as_bytes(), separator.as_bytes());
